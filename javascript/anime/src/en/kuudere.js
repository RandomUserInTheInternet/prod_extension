const mangayomiSources = [{
    ...
    "id": 847291854,
    "notes": "Kuudere - Anime streaming with ZenCloudz extraction",
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        var res = await this.client.get(
            `https://kuudere.to/api/top/anime?tab=today&limit=20`,
            this.getHeaders()
        );
    ...
    async getLatestUpdates(page) {
        ...
        var res = await this.client.get(
            `https://kuudere.to/recently-updated/__data.json${queryStr}`,
            this.getHeaders()
        );
    ...
    async search(query, page, filters) {
        var res = await this.client.get(
            `https://kuudere.to/search/__data.json?q=${encodeURIComponent(query)}&page=${page}`,
            this.getHeaders()
        );
        var rawData = JSON.parse(res.body);
        var dataArray = rawData.nodes[1].data;
        ...
        try {
            ...
            if (data.all_episodes && Array.isArray(data.all_episodes)) {
                for (var ep of data.all_episodes) {
                    var title = ep.titles && ep.titles.length > 0 ? ep.titles[0] : `Episode ${ep.number}`;
                    chapters.push({
                        name: title,
                        url: `${url}/${ep.number}`
                    });
            ...
            var res2 = await this.client.get(`https://kuudere.to/anime/${url}/__data.json`, this.getHeaders());
            if (res2.statusCode !== 200) return { name: "", imageUrl: "", description: "", genre: [], status: 0, chapters: [] };
            
            try {
                var rawData = JSON.parse(res2.body);
                var dataArray = rawData.nodes[1].data;
                var decoded = this.unflatten(0, dataArray);
                
                var info = null;
                if (Array.isArray(decoded)) {
                    for (var item of decoded) {
                        if (item && item.id === url) { info = item; break; }
                    }
                }
            } catch (parseError) {
            ...
            for (var link of data.episode_links) {
                ...
                if (originalUrl.includes("zencloudz.cc/e/")) {
                    try {
                        ...
                        var zenUrl = baseUrl + "/__data.json" + (params ? "?" + params : "");
                        ...
                        for (var n = 0; n < zenData.nodes.length; n++) {
                            if (zenData.nodes[n] && zenData.nodes[n].data) {
                                dataArray = zenData.nodes[n].data;
                            }
                        }
                        ...
                        // FIX: The seed parameter is extracted directly from the original seed, NOT the hash
